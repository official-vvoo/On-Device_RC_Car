...
object_dict = { 1:'stop_line', 2:'car', 3:'red_light', 4:'left_light', 5:'green_light',
                'stop_line':1, 'car':2, 'red_light':3, 'left_light':4, 'green_light':5}
...
class BoxLabel(PyQt5.QtWidgets.QLabel):
    ...
    def select_Qpen(self, obj, type=Qt.SolidLine ):
        ...
        if obj == 1 :            
            ...
            return QPen(QColor(0,0,255),Qt.SolidLine)
        elif obj == 4 :
            return QPen(QColor(0,255,255),Qt.SolidLine)
        elif obj == 5 :
            return QPen(QColor(255,255,0),Qt.SolidLine)
        elif obj == 6 :
            return QPen(QColor(255,0,255),Qt.SolidLine)
        elif obj == 7 :
            return QPen(QColor(70,150,120),Qt.SolidLine)
        elif obj == 8 : 
            return QPen(QColor(255,255,255),Qt.SolidLine)
        elif obj == 9 :
            return QPen(QColor(0,0,0),Qt.SolidLine)
        else:
    ...
    def save_rectangle(self, img, img_name, img_id):
        ...
        currentImageInfo = {
                            "file_name": img_name,
                            "id" : img_id,
                            "height" : 416, #img.shape[0],
                            "width" : 416, #img.shape[1]
                            }
...
class WindowClass(PyQt5.QtWidgets.QMainWindow, form_class):
    ...
    def keyPressEvent(self, e):
        ...
        if not control and isPrintable(e.key()):
            if self.image_folder != None :
                ...
                elif e.text() in [str(x) for x in range(1, 10)]:
    ...
    def img_name_Change(self):
        if self.List_img.currentItem() != None:
            ...
            self.img =  (np.fromfile(selected_image,np.uint8), IMREAD_COLOR)
            qimg = PyQt5.QtGui.QPixmap()
            qimg.load(selected_image)
            # transform = PyQt5.QtGui.QTransform().rotate(180)
            # qimg = qimg.transformed(transform)
            # qimg = qimg.scaled(640, 640)
