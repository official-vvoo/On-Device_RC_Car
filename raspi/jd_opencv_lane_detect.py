...
class JdOpencvLaneDetect(object):
    ...
    def get_steering_angle(self, img_lane, lane_lines, curr_steering_angle):
        if len(lane_lines) == 0:
            return 0, None
        ...
        curr_heading_image = display_heading_line(img_lane, curr_steering_angle)
        show_image("heading", curr_heading_image)

        return curr_steering_angle, curr_heading_image
        '''

        new_heading_image = display_heading_line(img_lane, new_steering_angle)
        show_image("heading", new_heading_image)

        return new_steering_angle, new_heading_image
...
def detect_edges(frame):
    ...
    '''
    lower_black1 = np.array([50, 50, 0])
    upper_black1 = np.array([85, 70, 100])
    ...
    '''
    lower_black2 = np.array([75, 38, 15])
    upper_black2 = np.array([100, 70, 37])

    mask2 = cv2.inRange(hsv, lower_black2, upper_black2)


    lower_black3 = np.array([80, 75, 40])
    upper_black3 = np.array([100, 112, 83])

    mask3 = cv2.inRange(hsv, lower_black3, upper_black3)
    '''

    '''
    lower_black4 = np.array([50, 50, 0])
    upper_black4 = np.array([98, 112, 100])
    ...
    show_image("grey", grey)

    low_threshold = 150
    high_threshold = 255
    mask = cv2.inRange(grey, low_threshold, high_threshold)
    '''
